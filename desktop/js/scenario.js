    ...
    addSubElement: function (_subElement) {
      ...
      switch (_subElement.type) {
        ...
        case 'for':
          ...
          if (isset(_subElement.expressions) && isset(_subElement.expressions[0])) {
            expression_for = _subElement.expressions[0]
          }
          retour += this.addExpression(expression_for)
          retour += '</div>'
          retour = this.addElButtons(retour)
          break
        ...
        case 'while':
          ...
          retour += '</div>'
          retour += '<div class="expressions" >'
          let expression_while = {
            type: 'condition'
          }
          if (isset(_subElement.expressions) && isset(_subElement.expressions[0])) {
            expression_while = _subElement.expressions[0]
          }
          retour += this.addExpression(expression_while)
          retour += '</div>'
          retour = this.addElButtons(retour)
          break
        ...
        case 'in':
          ...
          if (isset(_subElement.expressions) && isset(_subElement.expressions[0])) {
            expression_in = _subElement.expressions[0]
          }
          retour += this.addExpression(expression_in)
          retour += '</div>'
          retour = this.addElButtons(retour)
          break
        ...
        case 'at':
          ...
          if (isset(_subElement.expressions) && isset(_subElement.expressions[0])) {
            expression_at = _subElement.expressions[0]
          }
          retour += this.addExpression(expression_at)
          retour += '</div>'
          retour = this.addElButtons(retour)
          break
        ...
        case 'code':
          ...
          if (isset(_subElement.expressions) && isset(_subElement.expressions[0]) && typeof _subElement.expressions[0].expression == "string") {
            expression_code = _subElement.expressions[0]
            retour += '<div class="blocPreview">' + expression_code.expression.substring(0, 200).HTMLFormat() + '</div>'
          } else {
            retour += '<div class="blocPreview"></div>'
          }
          retour += '</div>'
          retour += '<div class="expressions">'
          retour += this.addExpression(expression_code)
          retour += '</div>'
          retour = this.addElButtons(retour)
          break
        ...
        case 'comment':
          ...
          let expression_comment = {
            type: 'comment'
          }
          if (isset(_subElement.expressions) && isset(_subElement.expressions[0])) {
            expression_comment = _subElement.expressions[0]
            if (typeof expression_comment.expression === 'object') {
              expression_comment.expression = JSON.stringify(expression_comment.expression, null, 2)
            }
            let txt = expression_comment.expression.substring(0, 200).HTMLFormat()
          ...
          retour += '</div>'
          retour += '<div class="expressions">'
          retour += this.addExpression(expression_comment)
          retour += '</div>'
          retour = this.addElButtons(retour)
          break
        ...
        case 'action':
          ...
          if (isset(_subElement.expressions) && isset(_subElement.expressions[0])) {
            let expression_action = _subElement.expressions[0]
            if (expression_action.type == 'element' && isset(expression_action.element.subElements) && isset(expression_action.element.subElements[0]) && isset(expression_action.element.subElements[0].expressions) && isset(expression_action.element.subElements[0].expressions[0])) {
              retour += '<div class="blocPreview">' + expression_action.element.subElements[0].expressions[0].expression.substring(0, 200).HTMLFormat() + '</div>'
            } else {
...
document.getElementById('scenariotab').addEventListener('click', function (event) {
  ...
  if (_target = event.target.closest('.bt_showElse')) {
    ...
    const elElse = _target.closest('.element').querySelector(':scope > .subElementELSE')
    if (icon == null) return
  ...
  if (_target = event.target.closest('.bt_collapse')) {
    ...
    for (const _item of changeThese) {
      const icon = _item.querySelector(':scope > i')
      if (open) { // -> Collapse!
        icon.removeClass('fa-eye').addClass('fa-eye-slash')
        _item.closest('.element').addClass('elementCollapse')
        _item.setAttribute('value', 1)
        _item.setAttribute('title', "{{Afficher ce bloc.<br>Ctrl+click: tous.}}")
        //update action, comment and code blocPreview:
        let txt, _el, id
        _item.closest('.element').querySelectorAll('.blocPreview').forEach(function (_blocPreview) {
          txt = '<i>Unfound</i>'
        ...
        _item.closest('.element').removeClass('elementCollapse')
        _item.setAttribute('value', 0)
        _item.setAttribute('title', "{{Masquer ce bloc.<br>Ctrl+click: tous.}}")
        jeeP.setEditors()
      }
    }
    jeedomUtils.initTooltips()
